...
def run(devices):
    ...
    for i in range(devices):
        ...
        pk_list.append(pk)
        if i == 0:  # Assign one as the actual signer
            signer_sk = sk

    # Simulation of Ring Signature (Signing with one key)
    sig = dilithium2.sign(signer_sk, b"ring-auth")

    # Simulation of Ring Verification (Server-side)
    start_server = time.perf_counter()
    for _ in range(devices):
        dilithium2.verify(pk_list[0], sig, b"ring-auth")
...
if __name__ == "__main__":
    ...
    print("\n=== Lightweight PQ Lattice Auth (Dilithium) Simulation ===\n")
    print(
        f"{'Devices':<10} | "
